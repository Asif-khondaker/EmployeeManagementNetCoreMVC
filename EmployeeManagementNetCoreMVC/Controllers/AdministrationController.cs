...
namespace EmployeeManagementNetCoreMVC.Controllers
{
    ...
    public class AdministrationController : Controller
    {
        ...
        public async Task<IActionResult> ManageUserRoles(string userId)
        {
            ...
            if (user == null)
            {
                ViewBag.ErrorMessage = $"User with id = {user.Id} can not be found";
                return View("NotFound");
            }
        ...
        public async Task<IActionResult> ManageUserRoles(List<UserRolesViewModel> model, string userId)
        {
            ...
            if (!result.Succeeded)
            {
                ModelState.AddModelError("", "Can't remove user existing role");
                return View(model);
            }


            result = await userManager.AddToRolesAsync(user,
                model.Where(x => x.IsSelected).Select(y => y.RoleName.ToString()));
            if (!result.Succeeded)
            {
                ModelState.AddModelError("", "Can't add selectd role to the user");
                return View(model);
            }
        ...
        public async Task<IActionResult> DeleteRole(string id)
        {
            ...
            else
            {
                ...
                catch (DbUpdateException ex)
                {
                    ...
                    ViewBag.ErrorTitle = $"{role.Name} role in used";
                    ViewBag.ErrorMessage = $"{role.Name} role can't be deleted as it has users assaigned!";
                    return View("Error");
                }
            }
